...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        att_wt = F.softmax(dot_product,dim=1)
        att_wts = self.dropout(att_wt)
        y = torch.bmm(att_wts,value)
        return y  
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            # Hint : If mask[i,j] = 0, we want softmax(QKT[i,j] + additive_mask[i,j]) to be 0
            # Think about what inputs make softmax 0.
            additive_mask = torch.where(attn_mask == 0, torch.tensor(-1e9), torch.tensor(0.0))
            additive_mask = torch.logical_not(additive_mask).unsqueeze(0)
            dot_product += additive_mask

        # apply softmax, dropout, and use value
        att_wt = F.softmax(dot_product, dim=1)
        att_wts = self.dropout(att_wt)
        y = torch.matmul(att_wts,value)
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        seq_dim = seq.size(-1)
        attn_output = attn_output[:, :, :seq_dim]
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mask = torch.triu(torch.ones(_len, _len, device=self.device), diagonal=1).bool()
        return mask
